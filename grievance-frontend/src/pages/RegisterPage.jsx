...
// ... (Keep your academicPrograms object exactly the same) ...
// ----- UPDATED DATA: 8 SCHOOLS OF CT UNIVERSITY -----
const academicPrograms = {
  "School of Engineering and Technology": [
    ...
    "B.Tech - Electronics & Comm (ECE)",
    "B.Tech - Robotics & Automation",
    "M.Tech - CSE / Civil / Mech",
    "BCA - General / Data Science",
    "MCA - General / Cyber Security",
...
function RegisterPage() {
  // ... (Keep all your state and handler logic exactly the same) ...
  ...
  const handleChange = (e) => {
    ...
    setFormData({ ...formData, [name]: processedValue });
    let error = "";
    if (name === "id") error = validateIdFormat(formData.role, processedValue);
    else if (name === "role") {
      const idError = validateIdFormat(processedValue, formData.id);
      setErrors(prev => ({ ...prev, id: idError }));
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (submissionData.role !== 'student') {
      submissionData.program = ''; 
      delete submissionData.studentType;
    }
    try {
      const res = await fetch("http://localhost:5000/api/auth/register", {
        method: "POST",
      ...
      setMsg("Registration successful! You can now login.");
      setStatusType("success");
      setFormData({ id: "", role: "", studentType: "current", fullName: "", email: "", phone: "", password: "", program: "", }); 
    } catch (err) {
      setMsg(err.message || "An error occurred during registration.");
      setStatusType("error");
    }
      ...
      <div className="login-form-section">
        <div className="form-wrapper">
          ...
          <form onSubmit={handleSubmit}>
            ...
            {/* STUDENT STATUS (Full Width if selected) */}
            {formData.role === 'student' && (
              <div className="input-group">
                <label>Student Status</label>
                <div className="radio-group-container">
